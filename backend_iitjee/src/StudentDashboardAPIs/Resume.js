...
router.get("/getResumedUserresponses/:studentId/:testCreationTableId", async (req, res) => {
    ...
    try {
        ...
        `SELECT subject_id,section_id,question_id,
        user_answer,question_status
         FROM iit_user_responses where student_registration_id = ? AND test_creation_table_id = ?;`,
        ...
        const subjects = rows.reduce((acc, { subject_id, section_id, question_id, user_answer, question_status }) => {
