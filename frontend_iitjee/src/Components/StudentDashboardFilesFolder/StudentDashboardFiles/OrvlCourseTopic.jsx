...
const OrvlCourseTopic = ({ topicid, onBack, studentId, courseCreationId }) => {
    ...
    const fetchCourseData = async () => {
      try {
        const response = await fetch(
          `${BASE_URL}/OrvlTopics/CourseTopic/${topicid}`
        );
        if (!response.ok) {
    ...
    try {
      ...
      selectedExercise.questions.forEach((question) => {
        ...
        const questionStatus = statusData.find(
          (status) =>
            status.exercise_question_id === question.exercise_question_id
        );
  ...
  const useranswervalue = async () => {
    const questionid =
      selectedExercise?.questions?.[currentQuestionIndex]?.exercise_question_id;
    let correctAns;

    if (
      selectedExercise?.questions?.[currentQuestionIndex]
        ?.exercise_question_type === "NATD"
    ) {
      correctAns =
        selectedExercise.questions[currentQuestionIndex].exercise_answer +
    ...
    try {
      ...
      if (data.length > 0) {
        ...
        if (selectedExercise?.questions?.[currentQuestionIndex]?.exercise_question_type ==="MSQ"){
          setSelectedOptions(result.userAnswer || []);
        } else{
          setUserAnswer(result.userAnswer || '');
        }
  ...
  const submitExerciseStatus = async (questionid) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/OrvlTopics/ExerciseQuestionstatus`,
        {
          method: "POST",
          headers: {
  ...
  const nextLectureOrExercise = async() => {
    ...
    const currentLectureIndex = courseData.lectures.findIndex(
      (lecture) =>
        lecture.orvl_lecture_name === selectedLecture.orvl_lecture_name
    );
  ...
  return (
    <div className={globalCSS.OrvlCourseTopicContainer}>
      ...
      <div className={globalCSS.OrvlCourseTopicHeader}>
        ...
        <div className={globalCSS.OrvlTopicName}>
          {courseData.orvl_topic_name}
        </div>
      </div>
